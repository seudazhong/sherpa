...
def test_knowledge_tool_policy() -> None:
    ...
    assert evaluate(reg.get("search_knowledge")) == "allow"
    assert evaluate(reg.get("list_knowledge_sources")) == "allow"
    assert evaluate(reg.get("add_knowledge_source")) == "allow"
    assert evaluate(reg.get("reindex_knowledge_source")) == "allow"
    assert evaluate(reg.get("remove_knowledge_source")) == "ask"  # destructive → approval
...
async def test_knowledge_tools_add_ingest_search() -> None:
    ...
    async with SessionLocal() as s:
        try:
            ...
            listed = await reg.get("list_knowledge_sources").execute(tctx, {})
            assert "fin.md" in listed.llm_content

            res = await reg.get("search_knowledge").execute(tctx, {"query": "预算 审批阈值"})

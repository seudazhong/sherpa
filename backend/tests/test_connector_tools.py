...
async def test_sync_connector_tool(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async with SessionLocal() as s:
        try:
            ...
            listing = await reg.get("connector_list").execute(tctx, {})
            assert str(cid) in listing.llm_content

            synced = await reg.get("connector_sync").execute(tctx, {"connector_id": str(cid)})
    ...
    async with SessionLocal() as s:
        try:
            ...
            provider = MockProvider(
                script=[
                    [
                        ToolCall(id="c1", name="connector_sync", args={"connector_id": str(cid)}),
                        Finish("tool_use"),
                    ],

...
async def test_materialize_edit_and_delta() -> None:
    ...
    run_id = "run-mech"
    await sbx.materialize(run_id, entries, await _mem_reader({ha: a, hb: b}))

    # Scratch holds ONLY the project bytes — no credential/.env leaked in.
    root = sbx.scratch_dir_for(run_id)
    assert (root / "src" / "a.txt").read_bytes() == a
    names = {p.name for p in root.rglob("*") if p.is_file()}
    assert names == {"a.txt", "b.txt"}

    sbx.apply_edit(run_id, sbx.ScratchEdit(path="src/a.txt", op="write", data=b"ALPHA2\n"))
    sbx.apply_edit(run_id, sbx.ScratchEdit(path="c.txt", op="write", data=b"new\n"))
    sbx.apply_edit(run_id, sbx.ScratchEdit(path="b.txt", op="delete"))

    delta = sbx.compute_delta(run_id, {"src/a.txt": ha, "b.txt": hb})
    ...
    sbx.cleanup(run_id)
    assert not sbx.scratch_dir_for(run_id).exists()


@pytest.mark.asyncio
async def test_path_escape_rejected() -> None:
    await sbx.materialize("run-esc", [], await _mem_reader({}))
    with pytest.raises(sbx.ScratchError) as ei:
        sbx.apply_edit("run-esc", sbx.ScratchEdit(path="../evil", op="write", data=b"x"))
...
async def test_delta_bounds(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    await sbx.materialize("run-bnd", [], await _mem_reader({}))
    sbx.apply_edit("run-bnd", sbx.ScratchEdit(path="a.txt", op="write", data=b"a"))
    sbx.apply_edit("run-bnd", sbx.ScratchEdit(path="b.txt", op="write", data=b"b"))
    delta = sbx.compute_delta("run-bnd", {})
    assert delta.over_bounds is True


def test_sweep_orphans() -> None:
    root = sbx.scratch_dir_for("keep").parent
    root.mkdir(parents=True, exist_ok=True)
    (root / "crash-1").mkdir()
    (root / "crash-2").mkdir()
    (root / "live").mkdir()
    removed = sbx.sweep_orphans(keep_run_ids={"live"})
    assert removed == 2
...
async def test_disabled_sandbox_is_its_own_named_reason(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    res = await sbx.run_in_scratch("run-disabled", "pytest -q")
    assert res.error == sbx.SANDBOX_DISABLED == "sandbox_disabled"
...
async def test_daemon_unreachable_is_its_own_named_reason(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    res = await sbx.run_in_scratch("run-daemon", "pytest -q")
    assert res.error == sbx.RUNTIME_DAEMON_UNREACHABLE == "runtime_daemon_unreachable"
...
async def test_image_missing_is_its_own_named_reason(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    res = await sbx.run_in_scratch("run-image", "pytest -q")
    assert res.error == sbx.RUNTIME_IMAGE_MISSING == "runtime_image_missing"
...
async def test_container_start_failure_is_its_own_named_reason(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    res = await sbx.run_in_scratch("run-start", "pytest -q")
    assert res.error == sbx.RUNTIME_START_FAILED == "runtime_start_failed"
...
async def test_output_retrieval_failure_is_its_own_named_reason(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    res = await sbx.run_in_scratch("run-transport", "pytest -q")
    assert res.error == sbx.RUNTIME_TRANSPORT_FAILED == "runtime_transport_failed"
...
async def test_unexpected_failure_carries_the_error_class(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    res = await sbx.run_in_scratch("run-weird", "pytest -q")
...
async def test_successful_run_reports_no_error(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    res = await sbx.run_in_scratch("run-ok", "echo hello")
...
async def test_run_sandbox_persists_edit_delta() -> None:
    ...
    async with SessionLocal() as s:
        try:
            ...
            assert not sbx.scratch_dir_for(str(out.runtime_session.id)).exists()
        finally:
            await s.rollback()
...
async def test_run_sandbox_missing_dependency_still_persists_edits(monkeypatch, caplog) -> None:  # type: ignore[no-untyped-def]
    ...
    async def _fake(scratch_dir: str, command: str) -> sbx.RunResult:
        return sbx.RunResult("", "sh: ruff: not found", 127, False)

    monkeypatch.setattr(sbx, "_execute_in_scratch", _fake)
    async with SessionLocal() as s:
        try:
            ctx = await _seed(s)
            project, wc = await _open_wc(s, ctx)

...
| **项目:编码运行时**（RuntimeSession open/exec/close + tar 传输 + 流式/取消）(ADR-047/ADR-048) | 🚧 **tar 传输 + 自建 runner 镜像已上线（P3）**；RuntimeSession 生命周期 ⬜ **P4** | ⬜ POST /projects/{id}/runtime · POST /runtime/{rid}/exec·/cancel · DELETE /runtime/{rid}（202 + SSE） | ⬜ `runtime_open`/`runtime_close`（allow）· `sh_exec`（**ask** + 安全命令 grants）· ~~`run_test`/`run_lint`~~（ADR-046 §决策10 已删） | ⬜ **Phase TR P5**（三栏工作台 + Run 控件 + 流式日志 + Stop + Runs tab） | non_idempotent_write | **ask**（白名单 allow） |
| **项目:文件读写**（宿主侧作用于工作副本 effective tree；沙箱不可用仍可用）(ADR-048) | ✅ `services/project_fs.py` + canonical overlay persist（P4.3） | （复用既有 tree/diff 端点 + P5 的树编辑） | ✅ `fs_list`/`fs_read`/`fs_grep`/`fs_write`/`fs_edit`/`fs_delete`（FULL-flat；P2 后补 descriptor） | ⬜ **Phase TR P5**（可编辑文件树，人的手改与 agent 的手改进同一个 overlay） | idempotent_write | allow（敏感路径 ask 在 P4 policy 子任务） |

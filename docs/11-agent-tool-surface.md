...
| 项目:详情·文件树·快照(ADR-037, W2a) | ✅ services/projects.py + project_fs.py | GET /projects/{id}·/tree·/snapshots | `fs_list`/`fs_read`（Project-bound effective tree） | ✅ 项目详情(只读树 + 快照 + 活动) | read_only | allow |
...
| 项目:任务工作副本（跨 turn 持久 working copy + overlay + lease/fence + head_generation CAS）(ADR-040/ADR-039/**ADR-047**) | ✅ 已上线；tar 注入匿名卷，不挂任何宿主路径 | ✅ working-copy + effective files + runtime + rebase-review | ✅ `fs_*` + `runtime_*` + `sh_exec`; old project tools deleted | ✅ Files/Chat/Workspace + Change Review + Run/stream/Stop/Runs/Artifacts/rebase（P5 真实点击） | idempotent_write | allow |
...
| **项目:编码运行时**（RuntimeSession open/exec/close + tar 传输 + 流式/取消）(ADR-047/ADR-048) | ✅ RuntimeSession service + hot container + boundary persist + recovery（P4.2/P4.4） | ✅ runtime open/exec/cancel/close + session-scoped runtime-execs history | ✅ `runtime_open`/`runtime_close`（allow）· `sh_exec`（ask + 安全命令 grants；FULL-flat） | ✅ 三栏 + Run/流式日志/Stop + Changes/Runs/Artifacts（P5.1/P5.3/P5.4，真实点击） | non_idempotent_write | ✅ **ask**（平台白名单 allow） |
| **项目:文件读写**（宿主侧作用于工作副本 effective tree；沙箱不可用仍可用）(ADR-048) | ✅ `services/project_fs.py` + canonical overlay persist（P4.3） | ✅ session-bound `/project-files` list/content PUT/DELETE（whole-file/refuse + hash/create-only） | ✅ `fs_list`/`fs_read`/`fs_grep`/`fs_write`/`fs_edit`/`fs_delete`（FULL-flat；P2 后补 descriptor） | ✅ Project-bound Chat 左栏可编辑 effective tree（P5.2） | idempotent_write | ✅ allow；敏感路径/递归删除 ask（P4.3a） |
...
> **✅ 2026-07-31 更新(Phase TR P3 已上线)**:同一行的 **Tool 单元格**从 ⚠️「实际必失败」改为 ✅ ——
> tar 传输替换了结构性坏掉的 bind mount,`project_run` 在 **Windows + Docker Desktop 真栈**上实测跑通
> (chat 里让模型写 `calc.py`+`test_calc.py` → `pytest -q` **真 exit 1** → 让它修 → **真 exit 0**;
> `ruff --version`→0.6.9;`git --version`→**127** 即 `environment_missing_dependencies`)。
> **UI 单元格仍是 ⬜,而且 P3 没有动它一下**:Run 控件/流式日志/Stop 是 **P5**,`createSandboxRun`
> 至今零调用点,上面那条"web 进程内同步执行"的阻塞也**原样存在**(P4 才把执行搬到 worker 的 202+SSE)。
> 因此 **B-8 依然 OPEN** —— 它在 **P5** 结束时才关闭。P3 期间的人工泳道验证走的是既有 Change Review
...
> **✅ 2026-08-03 P5 更新**：UI 单元格经真实点击置 ✅。Project-bound Chat 已有 editable
> effective tree、Run、流式日志、Stop、Changes/Runs/Artifacts、head-moved Rebase review 与
> 人工 Save；agent/human 两泳道和 390px 均通过。**B-8 CLOSED。**

**剩余 UI ⬜(其他能力):** 候选 Edit 抽屉 · 待办完成/编辑控件 · Connectors 连接页(需 OAuth 凭据)· 审批渲染器。Project 执行泳道已由 P5 交付。Knowledge/Projects/GitHub import 均已交付；W4 对外写仍需后续 ADR。

...
| 项目:GitHub 连接(凭据)(ADR-038, W2b · 契约/设计先行) | ⬜ 设计(services/github_source.py;AEAD vault·复用 connector 边界) | GET·POST·DELETE /connections/github(契约冻结) | ❌ 不给 agent(凭据边界) | ⬜ **W2b 静态稿**(连接状态) | idempotent_write | user-only |
| 项目:GitHub 一次性导入(选 repo/ref → 有界归档获取 → 不可变初始快照 + source OID)(ADR-038, W2b · 契约/设计先行) | ⬜ 设计(services/projects_import.py github 分支·durable job) | POST /projects/imports kind=github(契约:501→**202**);GET /projects/github/repos·/refs | ❌ 不给 agent(人工·跨凭据+不可信外部内容) | ⬜ **W2b 静态稿**(repo/ref 选择·导入进度·成功来源元数据·失败/重试) | idempotent_write(durable job) | user-only |
...
**剩余 UI ⬜(下一步补完的清单):** 候选 Edit 抽屉 · 待办完成/编辑控件 · Connectors 连接页(需 OAuth 凭据)· 审批渲染器(approve/reject + run 恢复,属 v1 收尾)。~~Knowledge(/library)页~~ **✅ KB5 已交付**(主页/来源详情/检索测试 + Chat 引用 chips + 无依据态;Sidebar/路由/API/Vite proxy 就位)。**Projects(/work/projects)= ADR-037 W2a 已实现并两栈验证**:上面 4 行的 service/REST/Tool/UI 单元格全部 ✅(空/模板/归档导入 + 详情只读树/快照/活动 + Open in Chat 不可变绑定 + `list/create/tree/read` agent 工具);生产导航已暴露(Sidebar「Projects」)。GitHub 导入 = **ADR-038 W2b 契约与设计先行**(本批):契约把 `POST /projects/imports kind=github` 从 501 改为 202、新增 repo/ref 选择端点 + GitHub connection 端点 + `project_sources`/`github_connections` 数据模型 + events `create_kind=github`,并交付 W2b **静态设计稿**(`design-workspace/github-import.html`:连接状态/repo·ref 选择/导入进度/成功来源元数据/失败·重试);**service/REST/UI 单元格仍 ⬜(设计态,生产实现待负责人审核,W2b 生产导航未暴露)**,GitHub 导入不给 agent(人工·跨凭据边界)。W3(sandbox)/W4(对外写)仍为后续 ADR。**这张表就是防"后端做了、前端忘了"的看板——每加一个能力,先在这里补行,UI 列不 ✅ 不收工。**

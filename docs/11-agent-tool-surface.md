...
| 项目:任务工作副本（跨 turn 持久 working copy + overlay + lease/fence + head_generation CAS）(ADR-040/ADR-039/**ADR-047**) | ✅ **已上线**（migration 0030；**Phase TR P3 起改为 tar 注入匿名卷，不挂任何宿主路径**） | GET /sessions/{id}/working-copy · GET/POST …/working-copies ✅ | ✅ **`project_run` 真的能跑了**（P3 实测：真容器真退出码；仍将由 ADR-048 的 `runtime_*`/`fs_*`/`sh_*` 在 **P4** 取代并删除） | ⚠️ **⬜**（Change Review 面板 ✅，但**仍没有 Run 控件**：`frontend/src/api.ts::createSandboxRun` 全前端**零调用点**）→ Phase TR **P5** | idempotent_write | allow |

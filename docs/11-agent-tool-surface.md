...
| **项目:文件读写**（宿主侧作用于工作副本 effective tree；沙箱不可用仍可用）(ADR-048) | ✅ `services/project_fs.py` + canonical overlay persist（P4.3） | （复用既有 tree/diff 端点 + P5 的树编辑） | ✅ `fs_list`/`fs_read`/`fs_grep`/`fs_write`/`fs_edit`/`fs_delete`（FULL-flat；P2 后补 descriptor） | ⬜ **Phase TR P5**（可编辑文件树，人的手改与 agent 的手改进同一个 overlay） | idempotent_write | ✅ allow；敏感路径/递归删除 ask（P4.3a） |

...
| 模型 provider:配置多来源(OpenAI/Anthropic/Gemini/DeepSeek/Qwen…；AEAD 密钥) (ADR-041) | ❌ **契约先行**(后续实现;data-model `model_providers` + api §10.8;kind=openai_compatible/anthropic/gemini) | GET/POST /providers · GET/PATCH/DELETE …/{id} · POST …/{id}/test·default · GET …/{id}/models | ❌ **不给 agent**(跨凭据边界=人工设置,同 GitHub 连接) | ⬜ **契约先行**(静态稿 `design-settings-models/index.html`;Settings「Models」面) | — | user-only |
| 模型 provider:全局默认 + 每会话切 model (ADR-041) | ❌ **契约先行**(sessions.model_provider_id/model 绑定) | POST …/{id}/default · GET/POST /sessions/{id}/model | ❌ **不给 agent**(model 选择=设置) | ⬜ **契约先行**(chat 顶栏 model 切换器) | — | user-only |

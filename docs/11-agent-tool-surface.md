...
| 项目:任务工作副本（跨 turn 持久 working copy + overlay + lease/fence + head_generation CAS）(ADR-040/ADR-039/**ADR-047**) | ✅ 已上线；tar 注入匿名卷，不挂任何宿主路径 | GET /sessions/{id}/working-copy · GET/POST …/working-copies ✅ | ✅ `fs_*` + `runtime_*` + `sh_exec`; `project_run/tree/read` 已删除（P4.6） | ⚠️ **⬜** Change Review ✅，但 Run/流式日志/Stop 仍是 P5 | idempotent_write | allow |
...
| **项目:编码运行时**（RuntimeSession open/exec/close + tar 传输 + 流式/取消）(ADR-047/ADR-048) | ✅ RuntimeSession service + hot container + boundary persist + recovery（P4.2/P4.4） | ✅ POST /projects/{id}/runtime · POST /runtime/{rid}/exec·/cancel · DELETE /runtime/{rid}（worker-owned 202） | ✅ `runtime_open`/`runtime_close`（allow）· `sh_exec`（ask + 安全命令 grants；FULL-flat） | ⬜ **Phase TR P5**（三栏工作台 + Run 控件 + 流式日志 + Stop + Runs tab） | non_idempotent_write | ✅ **ask**（平台白名单 allow） |

> **评审修订（2026-07-19）**：下文的介绍与定位描述 Sherpa 的**长期愿景**；v1 是其中收窄的首个切片：**自托管、单实例、单用户的 Gmail → Action 助理**，并保留基础 **Web 聊天**作为次要界面（ADR-022）。当前范围见[设计评审汇总](reviews/README.md)，交付阶段见[路线图](09-roadmap.md)。
...
> **评审修订（2026-07-19）**：以下 12 项仍是长期需求；v1 **纳入** #1（自托管一键部署）、#2（“多用户登录”收窄为单一 owner 登录）、#5（只读 Gmail 连接）、#7（仅待办/提醒调度；通用 cron 后置）、#8（分析后只生成 candidate，用户 accept/edit 后才成为正式 todo）、#11（仅 Web 通知/摘要邮件），另保留基础 **Web 聊天**作为次要界面；v1 **后置** #3 沙箱/代码、#4 每用户文件存储、#6 GitHub、#9 agentic email、#10 QQ/IM、#12 团队协作。

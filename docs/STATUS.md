...
## ▶ Next ready task
**Phase TR **P4** (RuntimeSession + `fs_*`/`sh_exec` + async worker-executed REST) — ✅ **P3 is COMPLETE and owner-accepted (2026-08-01)** and P2's catalog is deferred by owner decision, so P4's remaining hard dependency (P3's transport) is met.** Then **P5** (`/Project` three-column UI + Run/Stop), which is what actually closes **B-8**.

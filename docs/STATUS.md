...
## Where we are
The **design + contracts + runnable skeleton** are done. The v1 durable spine (M1) is **in progress**: persistence, event journal + outbox, Redis/SSE fan-out, effect idempotency, provider+tools, the bounded core loop, durable prompt admission, the REST auth + session/message surface, and the **credential vault (AEAD/KEK)** (#1–#11) are implemented and green. Next: observability (#12).

## Verified state
- **Backend** (`backend/`, via `uv`): `uv sync` ok · `uv run pytest` → **32 passed** (needs Postgres+Redis up; vault tests need neither) · `ruff check`+`format --check` clean · `mypy app` clean. `uv.lock` committed.
...
## ▶ Next ready task
**M1 #12 — Observability** (structured JSON logging with tenant/run/session correlation ids + secret redaction, request/run tracing hooks, `/metrics` or health surface as scoped). Wire the redaction helper (`app/security/redaction.py`) into the log formatter. See `docs/07-observability.md`, `docs/contracts/config-and-secrets.md §3.5`.

## In progress
**M1 — durable spine.** #1–#11 done (persistence, migrations, journal+outbox, Redis/SSE, effect idempotency, provider+mock, tools, core loop, durable prompt admission, REST auth+sessions+messages, credential vault AEAD/KEK). #12 next — structured logging + correlation + redaction so runs are debuggable without leaking secrets.

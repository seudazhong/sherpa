...
>
> Last updated: 2026-07-24 · Phase: **Milestones 1–5 complete**; **post-v1 P0–P2, Phase CRON (ADR-031), Phase APPROVALS (ADR-034), Phase SCHED-FIX (ADR-031 amendment), and Phase OBS-A (ADR-033 observability: gen_ai spans + real per-call tokens — closes item 0) all complete + verified**. Next: **Phase OBS-B (optional Phoenix backend) or the next roadmap milestone**. The shipped RAG is archival semantic notes, not a source-backed document knowledge base. v1 + v1 wrap-up + UI/UX backlog + the responsive **Quiet Work** redesign are done.
...
- **Verify** (OBS.V): full backend gate green (**187 passed**, ruff + `mypy app` clean, migration head **0025**). Manual `OTEL_ENABLED=true` in-memory-exporter run prints the tree `invoke_agent(loops=2, stop=completed) > chat(in/out/finish=tool_use) · execute_tool(get_time, success) · chat(in/out/finish=stop)` with real tokens, durable digest-only `model.*` events, 2 generations rows, and real trace totals (input=305/output=20, not chars/4). **Real-provider token check blocked by the local litellm proxy's own `No connected db` state (proxy-side, not Sherpa)** — usage parsing is unit-tested against a mocked SSE usage chunk; owner can manually confirm live tokens once the proxy has a DB/key. No Playwright (no user-facing UI in Phase A).

**Deferred:** Phase B (optional Phoenix container, reuse Postgres + OTLP exporter) · Phase C (evals/flywheel, evidence-gated). **Next: Phase OBS-B or the next roadmap milestone (owner's choice).**

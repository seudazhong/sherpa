...
## Verified state
- **Backend** (`backend/`, via `uv`): `uv sync` ok · `uv run pytest` → **92 passed** (needs Postgres+Redis up; vault/formatter/compaction/spill-unit tests need neither) · `ruff check`+`format --check` clean · `mypy app` clean. `uv.lock` committed. Schema at alembic `0014`.
...
**M2 + M-tools complete; v1 wrap-up landed** (context-fidelity fix + approval closure, both browser-verified). Deferred: agent observability (owner), M3 eval gate (ADR-024 → post-v1 #11). Next: UI/UX backlog (`ui-backlog.md`) or the **post-v1 milestones** in `09-roadmap.md` (memory → files → sandbox → …).

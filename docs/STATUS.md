...
| **Milestone 4 — QQ/IM inbound + IM approval renderer** — **now on the OFFICIAL QQ platform** (ADR-028: qq-botpy WebSocket; OneBot removed) | ✅ done (browser-verified: Connectors page manual-config → Connected + sealed secret; **real q.qq.com QR bind endpoint live** (real task_id + QR + poll); QQ simulate inbound→loop→reply with the real model) |

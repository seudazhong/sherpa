...
>
> Last updated: 2026-07-22 · Phase: **Milestones 1 (memory+RAG), 2 (files/MinIO), 3 (sandbox) & 4 (QQ/IM) complete + browser-verified**. v1 + v1 wrap-up + UI/UX backlog all done. Next: post-v1 **Milestone 5 agentic email** (`09-roadmap.md`).
...
## Verified state
- **Backend** (`backend/`, via `uv`): `uv sync` ok · `uv run pytest` → **114 passed** (needs Postgres+Redis up; MinIO for file tests; vault/formatter/compaction/spill-unit tests need none) · `ruff check`+`format --check` clean · `mypy app` clean (119 files). `uv.lock` committed. Schema at alembic `0017`.
...
| **Milestone 4 — QQ/IM inbound + IM approval renderer** (OneBot v11 channel + reply delivery + approve/reject over IM) | ✅ done (browser-verified with real model: IM chat round-trip; gated send_email approved over IM → resume 'resumed') |

**Milestones 1 (memory+RAG), 2 (files/MinIO), 3 (sandbox), 4 (QQ/IM) all DONE + browser-verified**, on top of v1 + v1 wrap-up + UI/UX backlog. Milestone 4: ADR-026; adapter-shaped IM channel (OneBot v11/aiocqhttp) — `POST /channels/qq/webhook` (HMAC-SHA1 + owner allowlist) reuses `admit_prompt` → bounded loop; IM threads map to existing `sessions` (`channel='qq'`, no new table/contract change); worker pushes a settled run's reply (+ pending-approval preview) back to QQ; **IM approval reuses the v1 base** (approve/reject reply → `resolve_approval(channel='qq')` → resume). Owner-authed `GET /channels` status + `POST /channels/qq/simulate` (human-lane test without a live bot) + `/channels/threads/{id}` transcript with in-thread approve/reject. `qq_kind='disabled'` default (RecordingQQClient, offline). **Deferred (real-account, manual acceptance):** actual QQ network delivery needs a live bot (`QQ_*` env). Deferred (contract): scheduled firings → QQ (frozen schedules CHECK); group messages; `qq-botpy` WS backend.

**▶ Next: Milestone 5 — agentic email** (AgentMail, ADR-013; owner provided an API key). Agent-owned mailbox identity: inbound email → loop, autonomous send, email-side action confirmation; unify `send_email` with the EmailSender seam (roadmap note). Real-account parts may be manual. Each: full tests + per-milestone Playwright where an account allows.

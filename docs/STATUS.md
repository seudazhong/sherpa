...
>
> Last updated: 2026-07-22 · Phase: **Milestones 1–5 (memory+RAG, files/MinIO, sandbox, QQ/IM, agentic email) all complete + browser-verified**. v1 + v1 wrap-up + UI/UX backlog all done. All requested post-v1 milestones done; next candidates in `09-roadmap.md` (#6 cron … #11 eval).
...
## Verified state
- **Backend** (`backend/`, via `uv`): `uv sync` ok · `uv run pytest` → **122 passed** (needs Postgres+Redis up; MinIO for file tests; vault/formatter/compaction/spill-unit tests need none) · `ruff check`+`format --check` clean · `mypy app` clean (120 files). `uv.lock` committed. Schema at alembic `0017`.
...
Milestone 4: ADR-026; adapter-shaped IM channel (OneBot v11/aiocqhttp). Deferred: scheduled firings → QQ (frozen schedules CHECK); group messages; `qq-botpy` WS backend; real QQ account (manual).

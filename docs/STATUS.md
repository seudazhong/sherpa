...
>
> Last updated: 2026-07-22 · Phase: **Milestones 1–5 (memory+RAG, files/MinIO, sandbox, QQ/IM, agentic email) all complete + browser-verified**. v1 + v1 wrap-up + UI/UX backlog + the responsive **Quiet Work** redesign are done. All requested post-v1 milestones done; next candidates in `09-roadmap.md` (#6 cron … #11 eval).
...
4. **R-SESSION-SEARCH research** — ⬜ queued: verify how Copilot CLI, Hermes, and comparable local agents persist/search/resume sessions, then design Sherpa's cloud-native Session Library, canonical/index split, recovery guarantees, and static HTML UX before any implementation. Task brief: [`research/session-search.md`](research/session-search.md).
5. **R-WORKSPACE-PRODUCT research** — ⬜ queued: treat `Files` as the backend storage seam; research and design the user-facing Workspace / Projects / Drive product, configurable per-user quota, GitHub import/sync, durable development state, sandbox save boundaries, storage accounting, and static HTML flows. Task brief: [`research/workspace-product.md`](research/workspace-product.md).
...
| **R-SESSION-SEARCH** cloud session persistence/search/resume research | ⬜ queued (`docs/research/session-search.md`) |
| **R-WORKSPACE-PRODUCT** personal drive/project workspace research | ⬜ queued (`docs/research/workspace-product.md`) |

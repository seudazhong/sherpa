...
## Verified state
- **Backend** (`backend/`, via `uv`): `uv sync` ok · `uv run pytest` → **90 passed** (needs Postgres+Redis up; vault/formatter/compaction/spill-unit tests need neither) · `ruff check`+`format --check` clean · `mypy app` clean. `uv.lock` committed. Schema at alembic `0014`.

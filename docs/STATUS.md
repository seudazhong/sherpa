...
## Verified state
- **Backend** (`backend/`, via `uv`): `uv sync` ok · `uv run pytest` green (needs Postgres+Redis up; MinIO for file/drive tests) · `ruff check`+`format --check` clean · `mypy app` clean. `uv.lock` committed. Schema at alembic **`0026`** (0019 run lease · 0020 session search · 0021 personal drive · 0022 files→drive · 0023 recurring schedules/general cron · 0024 pre-authorization grants · 0025 debug durability · 0026 embedding dim 1536→1024 for bundled ollama bge-m3).

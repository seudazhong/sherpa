...
## Phase W2b-DESIGN — Workspace Projects GitHub one-time import: 研究收敛 + 契约与设计先行 (ADR-038) — ✅ **DESIGN/CONTRACT-FIRST COMPLETE (2026-07-27); production impl awaiting owner review**
...
| W2b.1 | **(impl, ⬜ awaiting owner review)** Schema `0029` + models: `project_sources` + `github_connections` + `projects.source_status` widen + `project_import_jobs` github columns. `services/github_source.py` (ref→OID resolve, bounded tarball fetch via connection credential). | data-model §Projects W2b; ADR-019/038 | migration head 0029; credential AEAD round-trip; token never in project state; tests | ⬜ pending owner review |
| W2b.2 | **(impl, ⬜)** GitHub import durable job: extend `services/projects_import.py` with the `create_kind='github'` branch (claim → resolve ref→OID → bounded tarball fetch → reuse W2a safe expander → initial immutable snapshot w/ `source_oid` → atomic activate; failed ⇒ no snapshot, visible+deletable; idempotent; recovery tick). | events §2.10; config §1.6 | crash/replay safe; unsafe/oversized archive → import_failed; read-only retry idempotent; tests | ⬜ pending owner review |
| W2b.3 | **(impl, ⬜)** REST §10.6 (`kind='github'` 202 + repo/ref pickers + connection endpoints; server-side credential proxy, token never to client; CSRF; 409/422/501→202/502/507). **No new agent tool** (human-only import). | api §10.6; ADR-023 | REST green; token never leaves server; authz/CSRF; tests | ⬜ pending owner review |
| W2b.4 | **(impl, ⬜)** Frontend on `/work/projects`: GitHub connection panel + repo/ref picker (branch/tag/commit) + import progress + source-metadata detail; enable the "GitHub" create path (was disabled→W2b). Capability-matrix UI cells → ✅. | design-workspace/github-import.html; matrix §9 | build/lint green; renders; matrix UI ✅ | ⬜ pending owner review |
| W2b.V | **(impl, ⬜)** Verify: backend gate (alembic 0029 / ruff / mypy / pytest) + two-lane Playwright (human: connect → pick repo/ref → import → source metadata + failure/retry + 390px; agent: reads the imported project via existing `project_tree`/`project_read`) + UX pass. | AGENTS §2 | verified; UX notes | ⬜ pending owner review |

**W2b-DESIGN exit:** ADR-038 accepted; frozen contract deltas (data-model `project_sources`/`github_connections`/`source_status`/`project_import_jobs`; api §10.6; events §2.10; config `GITHUB_*` + §1.6) + capability-matrix rows (UI ⬜) + the production static draft (`design-workspace/github-import.html`, desktop + 390px) are in; **no production code, no migration, no exposed W2b navigation.** W2b implementation (W2b.1…W2b.V) starts after owner review. **Non-goals (each a later ADR):** task working copy + scratch-copy sandbox + change review (W3, gated on the ADR-025 revision + `docker.sock`/multi-user isolation hardening); GitHub sync/push/PR (W4).

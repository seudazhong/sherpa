...
| P2.0b | **Prose diet + description byte cap** (the other half of P2.0 — **not started**) | `backend/app/tools/*.py`, `backend/app/config.py`, new startup assertion | Descriptions are still **39% of the surface** (6,336 B of 16,153). Add `TOOL_DESCRIPTION_MAX_BYTES` (proposal: 160) enforced at startup **next to the name regex**, so prose cannot silently refill; trim the offenders (`project_run` 641 chars, `project_tree` 507, `search_knowledge` 398, `create_scheduled_task` 276). Test first: a too-long description raises at startup. |
| P2.0c | **Owner-decision deletions** (blocked on a decision, ideally on B-11 data) | — | `drive_make_folder` · `list_notifications` · `reindex_knowledge_source` · `create_daily_digest`; plus whether memory keeps **two** systems (KV + archival). |

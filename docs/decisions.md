...
| 2026-07-28 | 自带 ollama/bge-m3 该不该是默认 | ✅ **提为开箱即用的部署默认**：`ollama` 服务移出 `embeddings` profile 随栈默认起，compose + `.env.example` 的 `EMBEDDING_KIND` 默认 `ollama`（`http://ollama:11434`）；**`Settings` 代码默认仍 `mock`**，保住裸跑/测试离线确定。模型与维度不变（bge-m3 / 1024） | 修订 ADR-032（部署默认；受 ADR-036 依赖驱动）|
...
- **修订（2026-07-28，用户拍板"我要默认就有 ollama bge-m3"）——把「自带 ollama」从"可选 profile"提为「开箱即用的部署默认」**：本 ADR 原意就是"embedding 默认走自带 ollama"，但落地时把 `ollama` 藏在 compose 的 `embeddings` profile 后、`EMBEDDING_KIND` 部署默认写成 `mock`，导致**默认栈的语义记忆与 Knowledge base（ADR-036，检索强依赖 bge-m3）是哑的**——用户得先取消注释再加 `--profile` 才能用，属于"文档说默认 ollama、实际默认 mock"的自相矛盾。故：**① `ollama` 服务移出 profile，随核心栈默认启动**（首次启动后台拉取 bge-m3 ≈1.2 GB，落 `ollamadata` 卷，之后秒起）；**② compose 与 `.env.example` 的 `EMBEDDING_KIND` 默认 = `ollama`、`EMBEDDING_BASE_URL` = `http://ollama:11434`**；**③ `Settings.embedding_kind` 的代码默认仍是 `mock`**——不起 docker 的裸跑与 `uv run pytest` 必须离线确定（AGENTS.md §5 测试铁律：测试不调真模型），"部署默认"与"代码默认"是两层，刻意不同。`EMBEDDING_KIND=mock` 保留为显式离线开关。**不改**模型/维度选型（bge-m3 / 1024，换模型仍＝全量重嵌）。
